...
__device__ float dev_interpolate(
    ...
    float *gradx_output, float *grady_output)
{
    ...
    if (tid < total_pixels) {
        ...
        // Interpolate gradients using GPU interpolation
        float gx1 = dev_interpolate(gradx1_data, grad_ncols, grad_nrows, x1 + i, y1 + j);
        float gx2 = dev_interpolate(gradx2_data, grad_ncols, grad_nrows, x2 + i, y2 + j);
        float gy1 = dev_interpolate(grady1_data, grad_ncols, grad_nrows, x1 + i, y1 + j);
        float gy2 = dev_interpolate(grady2_data, grad_ncols, grad_nrows, x2 + i, y2 + j);
...
__global__ void compute_intensity_difference_kernel(
    const float *img1_data, const float *img2_data,
    int img_ncols, int img_nrows,
    float x1, float y1, float x2, float y2,
    int width, int height,
    float *imgdiff_output)
{
    int total_pixels = width * height;
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (tid < total_pixels) {
        ...
        // Interpolate images using GPU interpolation
        float g1 = dev_interpolate(img1_data, img_ncols, img_nrows, x1 + i, y1 + j);
        float g2 = dev_interpolate(img2_data, img_ncols, img_nrows, x2 + i, y2 + j);
        
        // Compute difference
        imgdiff_output[tid] = g1 - g2;
    }
}

// ============ GPU INTENSITY DIFFERENCE LIGHTING INSENSITIVE KERNEL ============

__global__ void compute_intensity_difference_lighting_insensitive_kernel(
    const float *img1_data, const float *img2_data,
    int img_ncols, int img_nrows,
    float x1, float y1, float x2, float y2,
    int width, int height,
    float *imgdiff_output)
{
    ...
    if (tid < total_pixels) {
        ...
        // First pass: compute sums for normalization (we'll do reduction separately)
        // For simplicity, we'll compute the normalization factors in a separate kernel
        // This is a simplified version - you might want to implement proper reduction
        __shared__ float s_sum1[256];
        __shared__ float s_sum2[256];
        __shared__ float s_sum1_sq[256];
        __shared__ float s_sum2_sq[256];
        
        int local_tid = threadIdx.x;
        s_sum1[local_tid] = 0.0f;
        s_sum2[local_tid] = 0.0f;
        s_sum1_sq[local_tid] = 0.0f;
        s_sum2_sq[local_tid] = 0.0f;
        
        // Each thread processes multiple pixels for the reduction
        for (int p = tid; p < total_pixels; p += blockDim.x * gridDim.x) {
            int pi = (p % width) - hw;
            int pj = (p / width) - hh;
            
            float pg1 = dev_interpolate(img1_data, img_ncols, img_nrows, x1 + pi, y1 + pj);
            float pg2 = dev_interpolate(img2_data, img_ncols, img_nrows, x2 + pi, y2 + pj);
            
            s_sum1[local_tid] += pg1;
            s_sum2[local_tid] += pg2;
            s_sum1_sq[local_tid] += pg1 * pg1;
            s_sum2_sq[local_tid] += pg2 * pg2;
        }
        ...
        for (int offset = blockDim.x / 2; offset > 0; offset >>= 1) {
            if (local_tid < offset) {
                s_sum1[local_tid] += s_sum1[local_tid + offset];
                s_sum2[local_tid] += s_sum2[local_tid + offset];
                s_sum1_sq[local_tid] += s_sum1_sq[local_tid + offset];
                s_sum2_sq[local_tid] += s_sum2_sq[local_tid + offset];
            }
            __syncthreads();
        }
        
        // Compute normalization factors (only thread 0 in block)
        float alpha = 1.0f, belta = 0.0f;
        if (local_tid == 0) {
            float mean1_sq = s_sum1_sq[0] / total_pixels;
            float mean2_sq = s_sum2_sq[0] / total_pixels;
            float mean1 = s_sum1[0] / total_pixels;
            float mean2 = s_sum2[0] / total_pixels;
        ...
        // Broadcast alpha and belta to all threads in block
        __shared__ float s_alpha[1];
        __shared__ float s_belta[1];
        if (local_tid == 0) {
            s_alpha[0] = alpha;
            s_belta[0] = belta;
        }
        ...
        // Compute normalized difference for this thread's pixel
        imgdiff_output[tid] = g1 - g2 * alpha - belta;
    }
}
    ...
    float *out_x, float *out_y, int *out_status)
{
    ...
    for (iteration = 0; iteration < max_iterations; iteration++) {
        ...
        for (int p = tid; p < patch_size; p += blockDim.x) {
            ...
            // Compute intensity difference using GPU interpolation
            float I1 = dev_interpolate(img1, width, height, x1 + i, y1 + j);
            float I2 = dev_interpolate(img2, width, height, x2 + i, y2 + j);
            float diff = I1 - I2;
            
            // Compute gradient sum using GPU interpolation
            float gx1 = dev_interpolate(gradx1, width, height, x1 + i, y1 + j);
            float gx2 = dev_interpolate(gradx2, width, height, x2 + i, y2 + j);
            float gy1 = dev_interpolate(grady1, width, height, x1 + i, y1 + j);
            float gy2 = dev_interpolate(grady2, width, height, x2 + i, y2 + j);
    ...
    _FloatWindow gradx, _FloatWindow grady)
{
  ...
  /* timing the compute_gradient_sum_kernel */
  {
    cudaEvent_t t0, t1;
    cudaEventCreate(&t0);
    cudaEventCreate(&t1);
    ...
    if (f) { fprintf(f, "compute_gradient_sum_kernel %g\n", (double)ker_ms); fclose(f); }
  }

  /* ensure device finished */
    ...
    _FloatWindow imgdiff)
{
    ...
    cudaMalloc((void**)&d_imgdiff_output, window_size);
    
    // Copy input data to device
    cudaMemcpy(d_img1, img1->data, img_size, cudaMemcpyHostToDevice);
    cudaMemcpy(d_img2, img2->data, img_size, cudaMemcpyHostToDevice);
    
    // Launch kernel
    int threadsPerBlock = 256;
    int blocksPerGrid = (total_pixels + threadsPerBlock - 1) / threadsPerBlock;
    
  /* time compute_intensity_difference_kernel */
  {
    cudaEvent_t t0, t1;
    cudaEventCreate(&t0);
    cudaEventCreate(&t1);
    cudaEventRecord(t0, 0);
    compute_intensity_difference_kernel<<<blocksPerGrid, threadsPerBlock>>>(
      d_img1, d_img2,
    ...
    cudaEventDestroy(t1);
    FILE *f = fopen("profiling/gpu_timing.txt", "a");
    if (f) { fprintf(f, "compute_intensity_difference_kernel %g\n", (double)ker_ms); fclose(f); }
  }
  cudaDeviceSynchronize();
    
    // Copy results back to host
    cudaMemcpy(imgdiff, d_imgdiff_output, window_size, cudaMemcpyDeviceToHost);
    
    // Free device memory
    cudaFree(d_img1);
    cudaFree(d_img2);
    cudaFree(d_imgdiff_output);
}
    ...
    float *out_x, float *out_y, int *out_status)
{
  ...
  /* timing the main track_features_kernel */
  {
    cudaEvent_t t0, t1;
    cudaEventCreate(&t0);
    cudaEventCreate(&t1);
    ...
    cudaEventDestroy(t1);
    FILE *f = fopen("profiling/gpu_timing.txt", "a");
    if (f) { fprintf(f, "track_features_kernel %g\n", (double)ker_ms); fclose(f); }
  }
