/**********************************************************************
Tracks 150 best features across all frames in Carsdataset (0001.pgm–0659.pgm)
**********************************************************************/
...
#endif
{
    ...
    sprintf(fnamein, "Carsdataset/%04d.pgm", startFrame);
    ...
    for (i = 1, frame = startFrame + 1; frame <= endFrame; i++, frame++) {
        sprintf(fnamein, "Carsdataset/%04d.pgm", frame);
        ...
        /* Swap pointers instead of re-reading img1/img2 each time */
